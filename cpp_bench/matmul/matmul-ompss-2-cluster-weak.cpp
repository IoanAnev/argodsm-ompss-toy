 ...
 *
 * This implementation uses weak and strong dependencies.
 */
...
label("master: correctness check `mat_c/r`")
void verify_results(
		double (*mat_c)[N],
		double (*mat_r)[N]);
/**
...
node(nanos6_cluster_no_offload)	\
label("master: verification matmul `mat_r`")
void matmul_ref(
		double (*mat_a)[N],
		double (*mat_b)[N],
		double (*mat_r)[N]);
...
init_matrices()
{
	...
	size_t chunk_per_node = node_chunk(N, BSIZE);

	for (int z = 0; z < N; z += chunk_per_node) {
		...
		#pragma oss task weakout(mat_a[z;chunk_per_node][0;NSIZE],	\
			...
			for (int i = z; i < z + chunk_per_node; i += BSIZE) {
				for (int j = 0; j < N; j += BSIZE) {
					...
					#pragma oss task				\
							...
							node(nanos6_cluster_no_offload)	\
							label("local: initialize block in `mat_a/b/c/r`")
...
int
node_chunk(const int& size, const int& task_size)
{
	/* Calculate data chunk for each node */
	const size_t nodes = nanos6_get_num_cluster_nodes();
	const size_t chunk = size / nodes;
	/* Make the appropriate number checks */
